...
xhr.open("GET", baseURL + "address/customer");
xhr.setRequestHeader("authorization", "Bearer " + access_token); //sessionStorage.getItem('access-token')
...
getPaymentMethods(){
  ...
  xhr.addEventListener("readystatechange", function () {  
    if (this.readyState === 4) {                                      
          let paymentMethods = JSON.parse(xhr.response);
          temp.setState({dataPayments: paymentMethods});
    }
})
...
//Invoke API call function from Commonfunctions
CommonFunctions.makeApiCall(
  url, 
  null,
  null,
  CommonFunctions.RequestTypeEnum.GET,
  null,
  responseText => {
    this.setState({
...
this.setState({saveAddressError:"dispNone"})
  //Validate the input feilds are empty
  //If empty, "required" text should be displayed
...
xhrCheckout.addEventListener("readystatechange", function () {
    if (this.readyState === 4) {              
        ...
        this.setState({saveOrderResponse : response});
        this.openMessageHandler();                      
    }
})  
